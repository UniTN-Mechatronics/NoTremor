...
- (instancetype)init
{
  ...
  if (self) {
    ...
    self.types = [NSMutableArray arrayWithObjects:MXLogDocument.class, MXMovieDocument.class, nil];
...
{
  if (path != nil)  self.folderPath = path;
  if (types != nil) self.types      = (NSMutableArray *)types;
  NSFileManager *manager = [NSFileManager defaultManager];
  NSArray *allItems = [manager contentsOfDirectoryAtPath:self.folderPath
                                                   error:nil];
  
  for (Class k in self.types) {
    NSMutableArray *docsList = [k filterFileNames:allItems atPath:self.folderPath];
